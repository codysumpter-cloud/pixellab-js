...
describe("Rotate 8 With Template", () => {
  test("should generate character in 8 directions", async () => {
    const client = Client.fromEnvFile(".env.development.secrets");

    const response = await retryWithBackoff(async () => {
      return await client.rotate8WithTemplate({
        description: "cool dragon",
    ...
    expect(response.images["south-east"]).toBeDefined();
    expect(response.images.east).toBeDefined();
    expect(response.images["north-east"]).toBeDefined();
    expect(response.images.north).toBeDefined();
    expect(response.images["north-west"]).toBeDefined();
    expect(response.images.west).toBeDefined();
    expect(response.images["south-west"]).toBeDefined();
    ...
    for (const [direction, image] of Object.entries(response.images)) {
      const outputPath = path.join(resultsDir, `8_rotations_${direction}.png`);
